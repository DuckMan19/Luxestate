...
function object() {
  ...
  console.log("Ten cua nguoi ban cuoi cung:" + user[user.friends.length - 1]);
  ...
  user.friends.push('Artman');
  //in ra độ dài của mảng bạn bè
  console.log("Do dai cua mang:" + user.friends.length);
}
object();

// bài tập 5: Cho mảng gồm các phần tử 2, 4, 5, 7, 32, 67, 30, 100. Trả về và in ra mảng mới gồm các phần tử có 
// giá trị gấp đôi giá trị của từng phần tử mảng đã khởi tạo ? Gợi ý dùng hàm map().
function array() {
  const arr = [2, 4, 5, 7, 32, 67, 30, 100];
  const double = arr.map(function (number, index) {
    return number * 2;
  });
  console.log(double)
}
array()
//=========================================================================Phần 3.2=======================================================================
// bài tập 1:Viết 1 hàm nhận vào 1 tham số là mảng và trả về ngẫu nhiên 1 phần tử của mảng ?
function random() {
...
//bài tập 2:Tạo 1 đối tượng student trống: Thêm thuộc tính name với giá trị là tên student? Thêm thuộc tính 
...
object()
// bài tập 3:Tạo 1 mảng gồm các object là các student gồm tên và điểm trung bình. Viết 1 hàm nhận vào 1 
...
function arr() {
  ...
  const result = studen.filter(function (curr) {
    return curr.avg > 5
  })
  console.log(result)
}
...
// bài tập 4:Tạo 1 mảng chứa các object có thuộc tính là tên các nhân viên và giá trị là tiền lương.Viết hàm
// trả về tổng tất cả lương của các nhân viên trong object ?
function getmoney() {
  const staff = [
    { name: 'akko', salary: 100 },
    { name: 'trieu', salary: 1 },
    { name: 'hao', salary: 10 }
  ]
  const kq = staff.reduce(function (total, money) {
    return total + money.salary
  }, 0)
  console.log(kq)
}
getmoney()

// bài tập 5:Cho mảng gồm các phần tử 2, 4, 5, 7, 32, 67, 30, 100. Tìm và in ra phần tử đầu tiên thoả điều kiện phần
// tử đó > 20 ?
function firstElement() {
  const arr = [2, 4, 5, 7, 32, 67, 30, 100]
  const result = arr.find(element => element > 20)
  console.log(result)
}
firstElement()

// bài tập 6 :Cho mảng diemso gồm các phần tử 5, 7, 8, 9, 3. Sử dụng hàm every để kiểm tra điểm số của học 
...
function point() {
  const diemso = [5, 7, 8, 9, 3]
  const result = diemso.every(points => points >= 5)
  if (diemso >= 5) {
    console.log("Đủ điểm đỗ")
  }
  console.log("Không đủ điểm đổ")
}
point()
