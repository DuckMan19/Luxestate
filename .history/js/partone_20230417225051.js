...
function Arr() {
  let arr = ["newbie", "react", "wedres"];
  // In ra số lượng các khoá học?
  let result = arr.map(function (currentvalues) {
    return currentvalues.length;
  });
  console.log(result.length);

  //In ra chỉ số index của mảng?
  arr.forEach(function (currentvalues, index) {
  ...
  arr.forEach(function name(stringname) {
    console.log((stringname = arr.join(",")));
  });
}
Arr();
